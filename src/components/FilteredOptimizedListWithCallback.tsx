...
export const FilteredOptimizedListWithCallback = () => {
  const [list, setList] = useState(initialList);
  const [item, setItem] = useState("");
  const [search, setSearch] = useState("");
  const [term, setTerm] = useState("");

  useRenderInfo("FilteredOptimizedListWithCallback");
  ...
  return (
    <div className="test-compo">
      <p>😎 `handleListClick` callback is memoized, so passing it as prop will not rerender children!</p>
      <div>
        <input
          value={item}
